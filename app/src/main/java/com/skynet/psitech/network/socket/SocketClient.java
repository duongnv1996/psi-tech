package com.skynet.psitech.network.socket;
...
public class SocketClient extends Service {
    ...
    public void sendMessage(String sendFrom, String idUser, String idHost, int idPost, String content, int type) {
        ...
        try {
            ...
            socket.emit("psi_chat", jsonObject);
    ...
    public void updateBooking(Booking booking) {
        JSONObject jsonObject = new JSONObject();
        try {
//            jsonObject.put(AppConstant.MSG, new Gson().toJson(booking));
//            jsonObject.put(AppConstant.MSG, new Gson().toJson(booking));
            socket.emit("psi_booking", new Gson().toJson(booking));
            LogUtils.e("psi_booking socket  to " + booking.getBooking_id());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//                intent.putExtra(SocketConstants.KEY_STATUS_CONNECTION, getString(R.string.connected));
                ...
                socket.off("send_psi_chat");
                socket.on("send_psi_chat", new Emitter.Listener() {
                    @Override
                    public void call(Object... args) {
                        Gson gson = new Gson();
                ...
                socket.off("send_psi_booking");
                socket.on("send_psi_booking", new Emitter.Listener() {
                    @Override
                    public void call(Object... args) {
                        LogUtils.e("send_psi_booking", args[0].toString());
                        Profile profile = AppController.getInstance().getmProfileUser();
                        if (profile != null && profile.getActive() == 1) {
                            Intent intent1 = new Intent();
                            intent1.setAction(SocketConstants.SOCKET_BOOKING);
                            intent1.putExtra(AppConstant.MSG, args[0].toString());
                            SocketClient.this.sendBroadcast(intent1);
                        }
