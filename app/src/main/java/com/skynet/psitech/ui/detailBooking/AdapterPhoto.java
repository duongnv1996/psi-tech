...
public class AdapterPhoto extends RecyclerView.Adapter<AdapterPhoto.ViewHolder> {
    ...
    public AdapterPhoto(List<String> list, Context context) {
        this.list = list;
    ...
    public void onBindViewHolder(@NonNull final ViewHolder viewHolder, final int i) {
        Glide.with(context).asBitmap().thumbnail(0.7f).load(list.get(i).contains("http") ? list.get(i) : new File(list.get(i))).into(viewHolder.img);
